...
export default function TokensBalanceShablon({
  LOTTERY_ADDRESS,
  txData,
  user,
  element,
  ...
  const checkApprove = async () => {
    try {
      ...
      console.log(
        parseInt(await tokenContract.allowance(user, LOTTERY_ADDRESS))
      );
    } catch (err) {
      console.log(err);
    }
  ...
  const deleteToken = async () => {
    const body = {
      address: user,
      deleteTokenAddress: element.address,
      chainId,
    };
    try {
    ...
    if (element.address != 0)
      return (
        <div className='relative'>
          <div
            className={
              !needApprove && needApprove != undefined
                ? 'shablonbalance shablonbalanceClicked USER_NoSelected'
                : 'shablonbalance USER_NoSelected'
            }
            style={{
              margin:
          ...
          >
            <div className='tokenImage'>
              {element.isImageAdded && (
                <Image
                  className='tokenpng'
                  src={`/tokens/${element.address}.png`}
                  width={32}
                  height={32}
                />
              )}
