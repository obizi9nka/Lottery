...
export default function NftsShablon({
  LOTTERY_ADDRESS,
  NFT_ADDRESS,
  setneedWallet,
  Data,
  ...
  const putOnSell = async () => {
    try {
      ...
      const tx = await contract.putOnSell(
        Data.edition,
        BigInt(cost * 10 ** 18)
      );
      await tx.wait();
  ...
  const buy = async () => {
    try {
      ...
      const tx = await contract.trigerSell(owner, Data.edition, {
        value: BigInt(Data.price * 10 ** 18),
      });
      await tx.wait();
      setIsTokenOnSell(false);
      setcost();
      ...
      if (!isConnected) issue = 'Connect Wallet';
      settxData({
        isPending: false,
        result: false,
        issue,
  ...
  let image = `/${tymblerNaNetwork ? 'imagesETH' : 'imagesBNB'}/${
    Data.edition
  }.png`;
  ...
  return (
    <div className='nftsShablon'>
      <div className='pad'>
        <div
          className={Data.edition == LotteryId ? 'bordernft ID' : 'bordernft'}
        >
          ...
          {loading ? (
            ...
            <div className='i'>
              {istokenOnSell ? (
                <button onClick={removeFromSell} className='mybutton '>
                  Remove from sale
                </button>
              ) : Mode ? (
                <div>
                ...
                <div className='pylt'>
                  <input
                    ...
                    style={{ width: '80px' }}
                    placeholder={`${tymblerNaNetwork ? 'ETH' : 'BNB'}`}
                  />
                  <button onClick={putOnSell} className='mybutton '>
                    Put On Sell
                  </button>
                </div>
              ) : wannaMessage ? (
            ...
            <div className='i'>
              {istokenOnSell ? (
                <div>
                  <button onClick={() => buy()} className='mybutton'>
                    Buy {Data.price} ETH
                  </button>
                </div>
              ) : Data.ismints ? (
              ...
              ) : LotteryId < Data.edition && chainId > 0 ? (
                <div>
                  {!isAutoEnterPined && (
                    <button className='mybutton' onClick={addToAutoEnter}>
                      Add to autoenter
                    </button>
                  )}
                  {isAutoEnterPined && (
                    <button
                      ...
                      onClick={deleteFromAutoEnter}
                    >
                      Delete from autoenter
                    </button>
                  )}
                </div>
              ) : LotteryId == Data.edition ? (
          ...
          <div
            className={Data.ismints ? 'dot absolute' : 'dot absolute '}
            style={{
              backgroundColor: Data.ismints ? '#00ff08' : 'rgb(255, 0, 0)',
            }}
          >
