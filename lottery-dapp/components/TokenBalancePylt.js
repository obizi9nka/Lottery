...
export default function TokensBalancePylt({
  LOTTERY_ADDRESS,
  NFT_ADDRESS,
  user,
  rokens,
  ...
  useEffect(() => {
    ...
    } else setplaceholder('Choose token');
  }, [TokenSelected, user]);
  ...
  const addTokensToBalance = async () => {
    ...
    try {
      ...
      const tx = await contract.addTokensToBalance(
        TokenSelected.address,
        BigInt(deposit * 10 ** decimals)
      );
      await tx.wait();
  ...
  useEffect(() => {
    ...
    try {
      for (let i = 0; i < deposit.length; i++) {
        if (deposit[i] != ',') _deposit += deposit[i];
        else _deposit += '.';
      }
    } catch (err) {
      console.log(err);
  ...
  const checkApprove = async (_deposit) => {
    if (TokenSelected != null)
      try {
        ...
        console.log(
          BigInt(await tokenContract.allowance(user, LOTTERY_ADDRESS)),
          BigInt(_deposit * 10 ** TokenSelected.decimals)
        );
        if (
          BigInt(await tokenContract.allowance(user, LOTTERY_ADDRESS)) <
          BigInt(_deposit * 10 ** TokenSelected.decimals)
  ...
  const withdrow = async () => {
    try {
      ...
      const tx = await contract.Withdrow(
        TokenSelected.address,
        BigInt(deposit * 10 ** decimals)
      );
      await tx.wait();
  ...
  return (
    <div
      className='depositAndWitdrow'
      style={{ marginBottom: rokens.length == 0 ? '10px' : null }}
    >
      {!needApprove && (
        <button
          onClick={addTokensToBalance}
          disabled={!isvalid}
          className='mybutton dinamic'
        >
          Deposit
        </button>
      )}
      ...
      <div className='depositvalue'>
        <input
          className='input dinamic'
          id='deposit/withdraw'
          style={{
            minWidth: '110px',
            color: deposit == '' ? 'white' : isvalid ? 'white' : 'red',
          }}
          disabled={TokenSelected == null}
      ...
      <button
        onClick={withdrow}
        disabled={!isvalid}
        className='mybutton dinamic withdrow'
      >
        Withdraw
      </button>
    </div>
