...
export default function Hard({
  ...
  const getAllLobbyes = async () => {
    ...
    await fetch('/api/getAllLobby', {
      ...
      setlobbyesActive(
        chainId == ETHid ? lobbys.lobbysETHActive : lobbys.lobbysBNBActive
      );
      setlobbyes(chainId == ETHid ? lobbys.lobbysETH : lobbys.lobbysBNB);
      setactiveMassiveLength(
        chainId == ETHid ? lobbys.lobbysETH.length : lobbys.lobbysBNB.length
      );
    });
  };
  ...
  const getAllNews = async () => {
    try {
      ...
      await fetch('/api/getUserData', {
        ...
        let temp;
        if (chain.id == ETHid) temp = tr.newsETH;
        else temp = tr.newsBNB;
        if (temp == null) return;
        const t = temp.split('&');
        t.pop();
        t.map((element) => {
    ...
    <div className=''>
      ...
      {isConnected && (
        <div
          className={
            activeMassive == 1
              ? 'menuLobby Selected pointer USER_NoSelected'
              : 'menuLobby menuHover pointer USER_NoSelected'
          }
          onClick={() => {
            setactiveMassive(1);
      ...
      <div
        className={
          activeMassive == 0
            ? 'menuGallery Selected pointer USER_NoSelected'
            : 'menuGallery menuHover pointer USER_NoSelected'
        }
        onClick={() => {
          setactiveMassive(0);
      ...
      <div
        className={
          isSlideShow
            ? 'Show Selected pointer USER_NoSelected'
            : 'Show menuHover pointer USER_NoSelected'
        }
        onClick={() => {
          StartSlideShow();
        }}
      ...
      {isConnected && (
        <div
          className={
            activeMassive == 2
              ? 'NewsButtonText Selected pointer USER_NoSelected'
              : 'NewsButtonText menuHover pointer USER_NoSelected'
          }
          onClick={() => {
            setactiveMassive(2);
      ...
      <div
        className='OFF pointer menuHover USER_NoSelected'
        onClick={() => {
          setVERSION(!VERSION);
        }}
      ></div>
      {!isSlideShow && (
        <div className='HARD_BACK_NEXT pointer USER_NoSelected'>
          <div className='HARD_areaBN '>
            ...
            <div
              ...
              onClick={() => {
                setSlideShow(
                  SlideShow + 1 < activeMassiveLength
                    ? SlideShow + 1
                    : activeMassiveLength - 1
                );
              }}
            >
              NEXT
      ...
      <div
        className='HARD_PopUp pointer USER_NoSelected'
        onClick={() => {
          settxData({
            isPending: null,
            result: null,
      ...
      >
        {Data.isPending && Data.result == null ? (
          <Loader loading={true} color={'#0d212a'} size={30} />
        ) : Data.result ? (
          <Image src='/succses.png' width={35} height={35} />
        ) : Data.result == false ? (
          <Image src='/wrong.png' width={35} height={35} />
        ) : (
          <div />
        )}
      </div>
        ...
        <div className='LeftMonitor USER_NoSelected'>
          <div className='HARD_lobbyCreate'>
            ...
            <button
              ...
              className='HARD_lobbyCreate_button HARD_ELEMENTS_LOBBY_CREATE pointer'
            >
              Create
            </button>
          </div>
      ...
      <div
        className={
          isSlideShow ? 'HARD_RigthMonitor active' : 'HARD_RigthMonitor'
        }
      >
        <HARD_LobbuShablon
          element={
            activeMassive == 0
              ? lobbyes[SlideShow]
              : activeMassive == 1
              ? lobbyesActive[SlideShow]
              : news[SlideShow]
          }
          activeMassive={activeMassive}
          isSHOW={true}
          isMonitor={true}
          EnterLobby={EnterLobby}
          imageFounded={imageFounded}
          index={SlideShow}
          filterModeScreen={filterModeScreen}
          needLigth={needLigth}
          setimageFounded={setimageFounded}
        />
      </div>
