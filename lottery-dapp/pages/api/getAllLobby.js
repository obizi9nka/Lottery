...
export default async function handler(req, res) {
  ...
  if (lobbysETH != []) {
    let _lobbysETHActive =
      UserData.LobbiesETH == null ? [] : UserData.LobbiesETH.split('_');
    let lobbyData = [];
    ...
    lobbysETH.forEach(async (element, index) => {
      const isEntered =
        element.creator == lobbyData[i]?.creator &&
        element.id == lobbyData[i]?.id;
      lobbysETH[index] = {
  ...
  if (lobbysBNB != []) {
    let _lobbysBNBActive =
      UserData.LobbiesBNB == null ? [] : UserData.LobbiesBNB.split('_');
    let lobbyData = [];
