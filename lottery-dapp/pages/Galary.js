...
export default function Home({
  ...
  const checkLotteryId = async () => {
    try {
      ...
      if (chainId == 31337) providerr = new ethers.providers.JsonRpcProvider();
      ...
      const body = {
        tokenId: id - 1,
        chainId,
        players: parseInt(dita.playersCount),
      };
  ...
  const setNFT = async () => {
    ...
    if (isConnected) {
      try {
        const body = {
          user: address,
          chainId: tymblerNaNetwork ? ETHid : BNBid,
        };
    ...
    try {
      await fetch('/api/get1000', {
        ...
        temp.forEach((element) => {
          if (element.isMinted) list.push(element.id);
    ...
    if (list != []) {
      try {
        ...
        if (tymblerNaNetwork)
          ...
          providerr = new ethers.providers.InfuraProvider(
            'sepolia',
            INFURA_KEY
          );
        if (chainId == 31337)
    ...
    if (isConnected) {
      try {
        ...
        let temp = data.map((element) => {
          if (parseInt(element) > id) return parseInt(element);
        });
    ...
    let notMinted = [];
    for (
      let i = 1, j = 0, x = 0, y = 0, mes = 0, autoEnterbd = 0, Tcount = 0;
      i < 1001;
      i++
    ) {
      if (list[j] === i) {
    ...
    for (
      ...
      if (message) mes++;
      const TransferCount =
        listTransferCount[Tcount]?.tokenId == i
          ? listTransferCount[Tcount].transfers
          : 0;
      if (TransferCount > 0 && listTransferCount.length > Tcount + 1) Tcount++;
      const body = {
        ismints,
        isowner,
        edition: i,
    ...
    sessionStorage.setItem(
      'chaindata',
      JSON.stringify({ tymblerNaNetwork, address })
    );
  ...
  useEffect(() => {
    ...
    if (
      !isSession ||
      tymblerNaNetwork !=
        JSON.parse(sessionStorage.getItem('chaindata'))?.tymblerNaNetwork ||
      address != JSON.parse(sessionStorage.getItem('chaindata'))?.address
    ) {
      setNFT();
    } else {
    ...
    <div className='areaNfts MAIN_MARGIG'>
      ...
      <div className='fiter'>
        <div className='anotherShit'>
          <button
            className='mybutton butound'
            onClick={() => changeState(false, false)}
          >
            Back
          </button>
            ...
            >
              ...
              <div className='searchIINFO'>
                <div style={{ display: 'grid' }}>
                  <div className='ttete'>s - sort</div>
                  <div className='ttete'>a - autoenter</div>
                  <div className='ttete'>m - my NFT</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <label className='switch zx'>
            <input
              ...
              onChange={() =>
                setALL_OR_MINTS(
                  ALL_OR_MINTS == 1 ? 2 : ALL_OR_MINTS == 2 ? 3 : 1
                )
              }
            />
            <span
          ...
          <select
            ...
            id='enougth'
            onClick={(e) => {
              localStorage.setItem('ENOUGTH', e.target.value);
              setcountOfRenderNfts(parseInt(e.target.value));
            }}
          >
            <option>5</option>
            <option>25</option>
          ...
          <button
            className='mybutton butound'
            onClick={() => changeState(true, false)}
          >
            Next
          </button>
        </div>
      ...
      {(ALL_OR_MINTS == 1
        ...
        ? tokensMints
        : tokensNotMints
      ).length > countOfRenderNfts && (
        <div className='areaFiter'>
          <div className='BackNext' style={{ width: 300 }}>
            <button
              className='mybutton'
              onClick={() => changeState(false, true)}
            >
              Back
            </button>
            <button
              className='mybutton'
              onClick={() => changeState(true, true)}
            >
              Next
            </button>
          </div>
