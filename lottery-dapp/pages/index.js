...
export default function Home({
  ...
  const infinity = async () => {
    ...
    else _provider = new ethers.providers.InfuraProvider('sepolia', INFURA_KEY);
    if (chain?.id == 31337) _provider = new ethers.providers.JsonRpcProvider();
    try {
  ...
  const Enter = async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(LOTTERY_ADDRESS, Lottery.abi, data);
      const tx = await contract.addToAutoEnter([
        parseInt(await contract.getLotteryCount()),
      ]);
      await tx.wait();
      setamIn(true);
      ...
      console.log(err);
      let issue;
      if (isConnected) issue = IssueMaker({ data: err.code, from: 'Enter' });
      else issue = 'Connect wallet';
      settxData({
        isPending: false,
        result: false,
        issue,
    ...
    <div className=''>
      ...
      <div className=''>
        ...
        <div className='index USER_NoSelected '>
          ...
          >
            <Image
              src={Images.lotteryIdMINUSMINUS}
              className='tttt'
              width={150}
              height={150}
            />
          ...
          >
            ...
            <div className='enterNftPlay'>
              {!amIn && (
                <button onClick={Enter} className='mybutton tobottom'>
                  Am In!
                </button>
              )}
              {amIn && (
                <button className='nftmintbuttonactive'>You're In!</button>
              )}
            </div>
          </div>
        ...
        {isneedShadow && (
          <div className='chooseWinner chooseWinnerText'>Choose winner</div>
        )}
        {isneedShadow && (
