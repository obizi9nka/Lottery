...
export default function Home({
  ...
  useEffect(() => {
    if (lobbyes == undefined) setimageFounded([]);
    else
      setimageFounded(
        lobbyes.map(() => {
          return true;
  ...
  useEffect(() => {
    if (lobbyes == undefined) setimageFoundedLobbyActive([]);
    else
  ...
  useEffect(() => {
    ...
    if (isSession) {
      ...
      if (address == _address && lobbys != undefined) {
        ...
        setlobbyesActive(
          tymblerNaNetwork ? lobbys.lobbysETHActive : lobbys.lobbysBNBActive
        );
  ...
  useEffect(() => {
    ...
    else if (countOfPlayers == '') setiscountOfPlayersvalid(undefined);
    else setiscountOfPlayersvalid(false);
  }, [countOfPlayers]);

  useEffect(() => {
    if (parseFloat(deposit) == deposit) setisdepositvalid(true);
    else if (deposit == '') setisdepositvalid(undefined);
    else setisdepositvalid(false);
  }, [deposit]);
  ...
  const createNewLobby = async () => {
    try {
      ...
      const id =
        1 + parseInt(await contract.getlobbyCountForAddressALL(address));
      const body = {
        user: address,
        token: token.address,
      ...
      const tx = await contract.createNewLobby(
        token.address,
        Deposit,
        countOfPlayers,
        { value: BigInt(10 ** 1) }
      ...
      await fetch('/api/createNewLobby', {
        ...
        const _data = {
          ...
          id: element.id,
          percent: parseInt(
            `${(element.nowInLobby / element.countOfPlayers) * 100}`.substring(
              0,
              2
            )
          ),
        ...
        chainId == ETHid
          ? temp.lobbysETH.push(_data)
          : temp.lobbysBNB.push(_data);
        setAll_LOBBYES(temp);
      });
      settxData({
        isPending: true,
      ...
      else issue = 'Deposit or Count of players';
      settxData({
        isPending: false,
        result: false,
        issue,
  ...
  const setSettingFOrFilter = () => {
    ...
    let token;
    rokens.map((element) => {
      if (element.symbol === tokenn) token = element.address;
    });
    const settings = {
      token,
      filterMode,
    ...
    setlobbyes([
      ...Filter(
        !tymblerNaNetwork ? ALL_LOBBYES.lobbysBNB : ALL_LOBBYES.lobbysETH,
        settings
      ),
    ]);
    try {
      ...
      for (let i = 0; i < stop; i++) {
        if (id == lobbyes[i].id && lobbyes[i].creator === creator) {
          if (parseInt(BigInt(loby.nowInLobby)) != 0) {
            ...
            if (chainId == ETHid) {
              ALL_LOBBYES.lobbysETH[i].nowInLobby = parseInt(
                BigInt(loby.nowInLobby)
              );
              ALL_LOBBYES.lobbysETH[i].players = address + '_';
            } else {
              ALL_LOBBYES.lobbysBNB[i].nowInLobby = parseInt(
                BigInt(loby.nowInLobby)
              );
  ...
  const isEnogth = (index) => {
    return (
      index >= startIndex &&
      index < parseInt(countOfRenderNfts) + parseInt(startIndex)
    );
  };
  ...
  return (
    <div className='MAIN_MARGIG'>
      <div className='new_active_lobbys'>
        <div className='newLobby'>
          <select
            onClick={(e) => {
              ...
              rokens.forEach((element) => {
                if (e.target.value === element.symbol) settoken(element);
              });
            }}
          ...
          <input
            ...
            id='deposit'
            placeholder='Deposit'
            onChange={(e) => setdeposit(e.target.value.toString())}
            style={{ color: isdepositvalid == false ? 'red' : null }}
          />
          <input
            className='input I'
            id='countofplayers'
            ...
            style={{ color: iscountOfPlayersvalid == false ? 'red' : null }}
          />
          <button onClick={createNewLobby} className='mybutton I'>
            Create New Lobby
          </button>
        </div>
        <div className='area'>
          {lobbyesActive &&
            lobbyesActive.map((element, index) => (
              <div
                className='LobbyShablon'
                style={{ borderColor: 'antiquewhite', cursor: 'default' }}
              >
                <div className='tokenAnd'>
                  <div className='tokeninlobbyshablon gridcenter'>
                    {imageFoundedLobbyActive[index] && (
                      <Image
                        className='tokenpng'
                        alt=''
                        onError={() => {
                          imageFoundedLobbyActive[index] = false;
                          setimageFoundedLobbyActive([
                            ...imageFoundedLobbyActive,
                          ]);
                        }}
                  ...
                  <div className='countofplayers gridcenter'>
                    <div style={{ position: 'relative' }}>
                      <div
                        className='circle'
                        style={{
                          backgroundImage: `conic-gradient(#fffeee ${element.percent}%, #232323 0)`,
                        }}
                      >
                ...
                <div className='predepositlobby'>
                  <div className='depositlobby'>
                    ...
                    <div>
                      Prize: {`${element.deposit * element.countOfPlayers}`}
                    </div>
                    <div>
                      Players: {element.nowInLobby}/{element.countOfPlayers}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div
        className='TryThis pointer USER_NoSelected'
        onClick={() => {
          setVERSION(!VERSION);
        }}
      >
      ...
      <div className='areafilterlobby'>
        <div className='FILTER'>
          ...
          <button
            onClick={() => setSettingFOrFilter()}
            className='mybutton choosetoken'
          >
            Filter
          </button>
        </div>
      ...
      <div className='Loby'>
        <button className='mybutton' onClick={() => changeState(false)}>
          Back
        </button>
        <select
          className='choosetoken mm'
          style={{ width: '65px' }}
          id='enougth'
          onClick={(e) => {
            localStorage.setItem('LobbyENOUGTH', e.target.value);
            setcountOfRenderNfts(e.target.value);
          }}
        >
          <option>20</option>
          <option>100</option>
        ...
        <button className='mybutton' onClick={() => changeState(true)}>
          Next
        </button>
      </div>
                      ...
                      <div className='predepositlobby'>
                        <div className='depositlobby'>
                          ...
                          <div>
                            ...
                            <strong>
                              {`${
                                element.deposit * element.countOfPlayers
                              }`.substring(0, 10)}
                            </strong>
                          </div>
                          <div>
          ...
          {!isLobbyFetched &&
            fakeMassive.map(
              (element, index) =>
                isEnogth(index + 1) && (
                  <div className='areaAraund'>
                    <div className={'LobbyShablon'}></div>
                  </div>
                )
            )}
        </div>
      ...
      <div className='areaFiter'>
        <div className='BackNext' style={{ width: 300 }}>
          <button className='mybutton' onClick={() => changeState(false)}>
            <a href='#Steps' style={{ color: 'white' }}>
              Back
            </a>
          </button>
